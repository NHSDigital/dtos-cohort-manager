...
{
    private readonly ILogger<NEMSSubscription> _logger;
    ...
    public NEMSSubscription(ILogger<NEMSSubscription> logger,
    ...
    public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = "subscriptions/nems")] HttpRequestData req)
    {
        try
        {
            ...
            {
                return CreateErrorResponse(req, HttpStatusCode.BadRequest, "NHS Number is required");
            }

            // Validate NHS Number format
            if (!IsValidNhsNumber(nhsNumber))
            {
                return CreateErrorResponse(req, HttpStatusCode.BadRequest, "Invalid NHS Number format");
            }
            ...
            {
                return CreateErrorResponse(req, HttpStatusCode.NotFound, "No matching patient found in PDS");
            }
            ...
            {
                return CreateErrorResponse(req, HttpStatusCode.BadGateway, "Failed to create subscription in NEMS");
            }
            ...
            if (!storageSuccess)
            {
                return CreateErrorResponse(req, HttpStatusCode.InternalServerError,
                    "Subscription created but failed to store locally");
            }
        ...
        {
            _logger.LogError(ex, "Error in NEMS subscription workflow");
    ...
    private async Task<bool> ValidateAgainstPds(string nhsNumber)
    {
        ...
        {
            _logger.LogError(ex, "PDS validation error");
            return false;
        }
    }
    ...
    private async Task<string> PostSubscriptionToNems(string subscriptionJson)
    {
        ...
        {
            _logger.LogError(ex, "NEMS subscription error");
            return null;
        }
    }
    ...
    private async Task<bool> StoreSubscriptionInDatabase(string nhsNumber, string subscriptionId)
    {
        ...
        {
            _logger.LogError(ex, "Database storage error");
            return false;
        }
    }
