namespace NHS.CohortManager.ParticipantManagementService;

using Microsoft.Azure.Functions.Worker;
using Microsoft.Azure.Functions.Worker.Http;
using Microsoft.Extensions.Logging;

using DataServices.Client;
using Common;
using System.Net;
using Model;

public class UnblockParticipant
{
    private readonly IDataServiceClient<ParticipantManagement> _participantManagementClient;
    private readonly IDataServiceClient<ParticipantDemographic> _participantDemographicClient;
    private readonly ILogger<UnblockParticipant> _logger;
    private readonly ICreateResponse _createResponse;
    private readonly IExceptionHandler _exceptionHandler;


    public UnblockParticipant(IDataServiceClient<ParticipantManagement> participantManagementClient, IDataServiceClient<ParticipantDemographic> participantDemographicClient, ILogger<UnblockParticipant> logger, ICreateResponse createResponse, IExceptionHandler exceptionHandler)
    {
        _participantManagementClient = participantManagementClient;
        _participantDemographicClient = participantDemographicClient;
        _logger = logger;
        _createResponse = createResponse;
        _exceptionHandler = exceptionHandler;
    }

    /// <summary>
    /// Takes in a http request that if the input is valid then it updates the blocked flag within participant management table to 0. Otherwise, adds an exception to the exception table.
    /// </summary>
    /// <param name="req">
    /// A Http request that should contain an NHS number, family name, date of birth and a screening ID.
    /// </param>
    /// <returns>
    /// A http status code. (200 - Ok / 404 - Not Found / 500 - Internal server error)
    /// </returns>
    [Function("UnblockParticipant")]
    public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequestData req)
    {
        _logger.LogInformation("Unblock Participant Called");
        long nhsNumberLong, screeningId;
        string? nhsNumber, dateOfBirth, familyName;

        nhsNumber = req.Query["NhsNumber"];
        nhsNumberLong = long.Parse(nhsNumber);

        screeningId = 1; //TODO Unhardcode this (Phase 2)

        try
        {
            if (!ValidationHelper.ValidateNHSNumber(nhsNumber)) 
            {
                _logger.LogError("Invalid NHS Number");
                await _exceptionHandler.CreateRecordValidationExceptionLog(nhsNumber, "", "Invalid NHS Number", screeningId.ToString(), req.ToString());
                return _createResponse.CreateHttpResponse(HttpStatusCode.BadRequest, req, "Invlaid NHS Number");
            }

            dateOfBirth = req.Query["DateOfBirth"];
            familyName = req.Query["FamilyName"];

            // Check participant exists in Participant Demographic table.
            ParticipantDemographic participantDemographic = await _participantDemographicClient.GetSingleByFilter(i => i.NhsNumber == nhsNumberLong && i.DateOfBirth == dateOfBirth && i.FamilyName == familyName);
            if (participantDemographic == null) 
            {
                _logger.LogError("Could not find participant");
                await _exceptionHandler.CreateSystemExceptionLogFromNhsNumber(new Exception("Could not find participant"), nhsNumber, "", screeningId.ToString(), req.ToString());
                return _createResponse.CreateHttpResponse(HttpStatusCode.NotFound, req);
            }
            
            ParticipantManagement participantManagement = await _participantManagementClient.GetSingleByFilter(i => i.NHSNumber == nhsNumberLong && i.ScreeningId == screeningId);
            participantManagement.BlockedFlag = 0;
            
            bool blockFlagUpdated = await _participantManagementClient.Update(participantManagement);
            if (!blockFlagUpdated) 
            {
                _logger.LogError("Failed to unblock participant");
                await _exceptionHandler.CreateSystemExceptionLogFromNhsNumber(new Exception("Failed to update blocked flag to unblocked"), nhsNumber, "", screeningId.ToString(), req.ToString());
                return _createResponse.CreateHttpResponse(HttpStatusCode.InternalServerError, req, "Failed to update unblocked flag to unblocked");
            }

            return _createResponse.CreateHttpResponse(HttpStatusCode.OK, req, "OK");
        }
        catch (Exception ex) 
        {
            _logger.LogError("Failed to unblock participant");
            await _exceptionHandler.CreateSystemExceptionLogFromNhsNumber(ex, nhsNumber, "", screeningId.ToString(), req.ToString());
            return _createResponse.CreateHttpResponse(HttpStatusCode.InternalServerError, req, "An unknown error has occured");
        }

    }
}