...
```bash
# Preview participants before deletion
curl -X POST "https://your-apim-instance.azure-api.net/api/PreviewParticipant" \
...
# Delete participants
curl -X POST "https://your-apim-instance.azure-api.net/api/DeleteParticipant" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
    "nhsNumber": "1234567890",
...
curl -i -X POST "https://your-apim-instance.azure-api.net/api/PreviewParticipant" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
...
curl -X POST "https://your-apim-instance.azure-api.net/api/GetParticipant" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
    "nhsNumber": "1234567890",
...
# Block participant
curl -X POST "https://your-apim-instance.azure-api.net/api/BlockParticipant" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
    "nhsNumber": "1234567890",
...
# Unblock participant
curl -X POST "https://your-apim-instance.azure-api.net/api/UnblockParticipant?nhsNumber=1234567890" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"

# Get participant with response headers
curl -i -X POST "https://your-apim-instance.azure-api.net/api/GetParticipant" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d @participant.json
```
...
### DeleteParticipant
- Deletions per day
- Preview vs actual delete ratio
- 404 rate (no matching participants)
- Multiple record deletions
- Average participants deleted per request

### ManageParticipant
- Messages processed per hour
- Add vs update ratio
- Blocked participant rejection rate
- Data service success rate
- Queue send success rate
- Exception creation frequency

### ManageServiceNowParticipant
- ServiceNow participants processed per day
- PDS validation success rate
- Name match failure rate
- VHR participant ratio
- NEMS subscription success rate
- ServiceNow message type distribution
- Blocked participant attempt rate

### UpdateBlockedFlag
- Block requests per day
- Unblock requests per day
- GetParticipant usage (safety checks)
- 404 rate (participant not found)
- PDS lookup frequency
...
### Deletion Safety
1. **Always preview before delete** - Use PreviewParticipant endpoint first
2. **Verify three-point match** - Ensure NHS number, family name, and DOB all match
3. **Audit deletions** - All deletions are logged for compliance
4. **Multiple record awareness** - One request can delete multiple records if they all match

### Blocking Participants
1. **Use GetParticipant first** - Preview participant before blocking
2. **Document reason** - Keep audit trail of why participant was blocked
3. **Monitor blocked attempts** - Track participants trying to be processed while blocked
4. **Unblock carefully** - Unblocking affects all screening services

### ServiceNow Integration
1. **Monitor PDS failures** - High 404 rate indicates data quality issues
2. **Track name mismatches** - Indicates ServiceNow vs PDS data discrepancies
3. **VHR flag persistence** - Once set, VHR flag is permanent (by design)
4. **NEMS subscription monitoring** - Failures are logged but non-blocking

### Service Bus Processing
1. **Exception handling** - All exceptions logged with context
2. **Blocked participant handling** - Creates exceptions for tracking
3. **Idempotency** - Updates are idempotent (can be safely retried)
4. **Queue dependencies** - Ensure downstream topics are healthy
