...
curl -X GET "https://your-apim-instance.azure-api.net/api/RetrievePdsDemographic?nhsNumber=1234567890" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"

# Retrieve with source file tracking
curl -X GET "https://your-apim-instance.azure-api.net/api/RetrievePdsDemographic?nhsNumber=1234567890&sourceFileName=batch_001.parquet" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"
...
curl -X POST "https://your-apim-instance.azure-api.net/api/DurableDemographicFunction_HttpStart" \
...
curl -X POST "https://your-apim-instance.azure-api.net/api/GetOrchestrationStatus" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '"orchestration-instance-id"'

# Using durable functions status endpoint (from 202 response)
curl -X GET "https://your-apim-instance.azure-api.net/runtime/webhooks/durabletask/instances/orchestration-instance-id" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"
```
...
curl -X POST "https://your-apim-instance.azure-api.net/api/Subscribe?nhsNumber=1234567890" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"

# Create multiple CAAS subscriptions
curl -X POST "https://your-apim-instance.azure-api.net/api/SubscribeMany" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '["1234567890", "9876543210", "1111111111"]'

# Check CAAS subscription status
curl -X GET "https://your-apim-instance.azure-api.net/api/CheckSubscriptionStatus?nhsNumber=1234567890" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"

# Unsubscribe (stub)
curl -X POST "https://your-apim-instance.azure-api.net/api/Unsubscribe?nhsNumber=1234567890" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"

# Query subscription data service
curl -X GET "https://your-apim-instance.azure-api.net/api/NemsSubscriptionDataService" \
  -H "Ocp-Apim-Subscription-Key: your-key-here"
```
...
### RetrievePDSDemographic
- PDS API calls per hour
- 404 rate (not found + restricted)
- Confidentiality code "R" frequency
- Bearer token refresh rate
- Database upsert success rate
- Average PDS response time

### DemographicDurableFunction
- Orchestrations started per hour
- Orchestration success/failure rate
- Average orchestration duration
- Retry counts and patterns
- Records inserted per orchestration
- Failed orchestrations requiring manual intervention

### ManageCaasSubscription
- New subscriptions per day
- Existing subscription check rate
- MESH send success rate
- Database save success rate
- Batch subscription size distribution
- MESH handshake success (nightly)

### ManageNemsSubscription
- New subscriptions per day
- Unsubscribe requests per day
- NEMS API success rate
- Subscription lookup performance
