...
public class ExtractData
{
    ...
    public ExtractData(ILogger<ExtractData> logger, ICreateResponse createResponse, ICallFunction callFunction)
    {
        _logger = logger;
    ...
    public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req)
    {
        ...
        try
        {
            if (req.Method == "POST")
            {
                ...
                var res = await _callFunction.SendPost(Environment.GetEnvironmentVariable("ParticipantDataServiceURI"), JsonSerializer.Serialize(participantData));

                if (res.StatusCode != HttpStatusCode.OK)
                {
                    _logger.LogInformation("Extract function failed");
                    return _createResponse.CreateHttpResponse(res.StatusCode, req);
                }
            }
            else
            {
                var functionUrl = Environment.GetEnvironmentVariable("ParticipantDataServiceURI");
