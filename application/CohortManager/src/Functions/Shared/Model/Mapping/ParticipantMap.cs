...
public class ParticipantMap : ClassMap<Participant>
{
    public ParticipantMap()
    {
        ...
        Map(m => m.PrimaryCareProviderEffectiveFromDate).TypeConverterOption.Format(DateFormats.Iso8601).Name("Primary Care Provider Business Effective From Date");
        Map(m => m.CurrentPosting).Name("Current Posting");
        Map(m => m.CurrentPostingEffectiveFromDate).TypeConverterOption.Format(DateFormats.Iso8601).Name("Current Posting Business Effective From Date");
        Map(m => m.PreviousPosting).Name("Previous Posting");
        Map(m => m.PreviousPostingEffectiveFromDate).TypeConverterOption.Format(DateFormats.Iso8601).Name("Previous Posting Business Effective To Date");
        ...
        Map(m => m.MobileNumberEffectiveFromDate).TypeConverterOption.Format(DateFormats.Iso8601).Name("Telephone Number (Mobile) Business Effective From Date");
        Map(m => m.EmailAddress).Name("E-mail address (Home)");
        Map(m => m.EmailAddressEffectiveFromDate).TypeConverterOption.Format(DateFormats.Iso8601).Name("E-mail address (Home) Business Effective From Date");
