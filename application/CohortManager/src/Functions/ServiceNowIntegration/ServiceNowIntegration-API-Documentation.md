...
curl -X POST "https://your-apim-instance.azure-api.net/api/servicenow/receive" \
...
curl -i -X POST "https://your-apim-instance.azure-api.net/api/servicenow/receive" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
...
curl -X PUT "https://your-apim-instance.azure-api.net/api/servicenow/send/CASE123456" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
    "messageType": "Success"
...
curl -X PUT "https://your-apim-instance.azure-api.net/api/servicenow/send/CASE789012" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
...
curl -X PUT "https://your-apim-instance.azure-api.net/api/servicenow/send/CASE456789" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
...
# Include response headers
curl -i -X PUT "https://your-apim-instance.azure-api.net/api/servicenow/send/CASE123456" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
...
- Message type distribution (Success/UnableToAdd/InProgress)
- ServiceNow API success rate
- Average response time from ServiceNow
- Failed message retries

### ServiceNowCohortLookup
- Daily execution success
- New cases processed per day
- Match success rate (processed/total)
- Cases with no matching participant
- Average processing time
- Yesterday's participant count
...
### Receiving ServiceNow Requests
1. **Validate early** - Request validation happens before database save
2. **Always save case** - Create ServicenowCase record before queueing
3. **Monitor queue send** - Queue failures are critical (case created but not processed)
4. **Log case numbers** - Essential for debugging and support
5. **Track validation failures** - High 400 rate indicates ServiceNow data quality issues

### Sending ServiceNow Messages
1. **Use correct message type** - Success vs Unable vs InProgress have different outcomes
2. **Monitor ServiceNow API** - Track failed calls to ServiceNow
3. **Log case context** - Include case number in all logs
4. **Handle API failures** - ServiceNow API failures should trigger alerts
5. **Message idempotency** - Safe to retry sends if uncertain about success

### Daily Reconciliation
1. **Monitor unmatched cases** - Cases that remain "New" after processing
2. **Track match rate** - Low match rate indicates processing issues
3. **Yesterday's scope** - Only matches participants inserted yesterday (by design)
4. **Manual investigation** - Cases not matched may need manual review
5. **Execution monitoring** - Ensure daily job completes successfully
