...
curl -X POST "https://your-apim-instance.azure-api.net/api/StaticValidation" \
...
curl -X POST "https://your-apim-instance.azure-api.net/api/LookupValidation" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
    "NewParticipant": {
...
curl -i -X POST "https://your-apim-instance.azure-api.net/api/LookupValidation" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
...
curl -X POST "https://your-apim-instance.azure-api.net/api/RemoveValidationExceptionData" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
    "nhsNumber": "1234567890",
...
curl -i -X POST "https://your-apim-instance.azure-api.net/api/RemoveValidationExceptionData" \
  -H "Content-Type: application/json" \
  -H "Ocp-Apim-Subscription-Key: your-key-here" \
  -d '{
    "nhsNumber": "9876543210",
...
- Average rules executed per validation
- Rule file load time

### LookupValidation
- Validations per hour
- Database lookup frequency
- Most common lookup failures
- Average database queries per validation
- Lookup cache hit rate

### RemoveValidationException
- Exceptions removed per day
- Successful removals vs not found
- Exception removal patterns
- Average exceptions per participant
...
### Static Validation
1. **Test rule changes** - Validate rule files before deployment
2. **Monitor rule failures** - Track which rules fail most often
3. **Keep rules modular** - One rule per validation check
4. **Document rules** - Include clear error messages
5. **Version rule files** - Track changes to validation logic

### Lookup Validation
1. **Cache reference data** - Minimize database queries
2. **Handle missing lookups** - Graceful failure for invalid codes
3. **Monitor lookup performance** - Track database query times
4. **Keep lookup data current** - Regular updates to reference tables
5. **Index lookup tables** - Optimize database performance

### Exception Management
1. **Document removal reasons** - Track why exceptions are removed
2. **Audit exception lifecycle** - From creation to removal
3. **Monitor exception trends** - Identify systematic issues
4. **Validate before removing** - Ensure issue is resolved
5. **Re-validate after removal** - Confirm participant now passes
