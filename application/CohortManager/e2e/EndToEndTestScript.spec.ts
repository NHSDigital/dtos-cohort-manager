...
test('Upload Add file to Azure Blob and check database', async () => {
    ...
    const recordFromDB = await checkDatabase(2);
...
test('Upload Add file to Azure Blob and expect it to error in database with error codes 36, 3645 and not go to cohort', async () => {
    ...
    await delay(20000);
    // Check that the cohort distribution has made it to the database via an HTTP API call.
    const itemsFromDatabase = await checkDatabase(5);
...
test('Upload update file and except it be in the database', async () => {
  ...
  (await checkDatabase(5)).forEach((item) =>{
...
test('Upload update file and except it not to be in database and error 22 in error table', async () => {
  // Initialize the Azure Blob Service Client and get the container and blob clients
  const blobServiceClient = BlobServiceClient.fromConnectionString(connectionString);
  const containerClient = blobServiceClient.getContainerClient(containerName);
  const blobName = "AMENDED_1B8F53_-_CAAS_BREAST_SCREENING_COHORT.parquet";
...
test('Upload file with 2 records and except 2 records to be returned from API', async () => {
...
test('Upload file with 300 records and except 300 records to be returned from API', async () => {
  ...
  var hasMoreThan500Records = (await checkDatabase(300)).length >= 300

  expect(hasMoreThan500Records).toBeTruthy()
});


async function checkDatabase(rowCount): Promise<[]> {
