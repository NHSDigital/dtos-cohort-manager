...
module "nic_diagnostic_policy_assignment" {
  for_each = var.regions

  source = "../../../dtos-devops-templates/infrastructure/modules/policy/policy-assignments"

  assignment_name                = "${module.regions_config[each.key].names.policy-assignment}-nic-diag"
  display_name                   = "Enforce NIC Diagnostic Settings"
  policy_definition_id           = module.nic_diagnostic_policy[each.key].policy_definition_id
  policy_assignment_scope        = "/subscriptions/${var.TARGET_SUBSCRIPTION_ID}"
}
