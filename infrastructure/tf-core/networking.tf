...
module "policy_assignment" {
  ...
  name                       = "deploy diagnostic settings for network interfaces to log analytics workspace [${azurerm_resource_group.rg_private_endpoints[each.key].name}]"
  log_analytics_workspace_id = data.terraform_remote_state.audit.outputs.log_analytics_workspace_id[local.primary_region]
  resource_group_id          = azurerm_resource_group.rg_private_endpoints[each.key].id
