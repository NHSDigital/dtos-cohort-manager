...
import { receiveParticipantViaServiceNow, invalidServiceNowEndpoint, getRecordsFromParticipantManagementService, getRecordsFromParticipantDemographicService } from '../../../../api/distributionService/bsSelectService';
...
test.describe.serial('@regression @service_now @api Verify Add Participant data', async () => {
  ...
  test.only('@DTOSS-10006 Add - Verify Participant Record: Happy Path case of data match', async ({ request }) => {
    ...
    await test.step('Check data in Participant Management table', async () => {
      ...
      //expect(response.data[0].RecordType).toBe("ADD");
      expect(response.data[0].NHSNumber).toBe(9000000009);
    });

    await test.step('Check data in Participant Demographic table', async () => {
