...
function buildDeletePayload(nhsNumbers: string[], input: any[]) {
  ...
  const isoDob = /^\d{8}$/.test(dob) ? `${dob.slice(0,4)}-${dob.slice(4,6)}-${dob.slice(6,8)}` : dob;
  return {
    NhsNumber: nhsNumbers[0],
    FamilyName: r0.family_name,
    DateOfBirth: isoDob,
...
/** Wait for any PM row to appear for this NHS no */
...
test.describe.serial('@regression @e2e @epic4b-2622', () => {
  ...
  test('@DTOSS-7610-01 ADD: blocked not processed to cohort', async ({ request }, testInfo) => {
    const [validationsRaw, nhsNumbersRaw, _parquet, inputRaw, testFilesPath] = await getApiTestData(testInfo.title);
    const validations = asArray(validationsRaw);
    const nhsNumbers = asArray(nhsNumbersRaw as any).map(String);
    const inputParticipantRecord = asArray(inputRaw);

    await cleanupDatabaseFromAPI(request, nhsNumbers);

    const parquet = await createParquetFromJson(nhsNumbers, inputParticipantRecord, testFilesPath, 'ADD', false);
  ...
  test('@DTOSS-7614-01 AMENDED: blocked not processed to cohort', async ({ request }, testInfo) => {
    ...
    const parquet = await createParquetFromJson(nhsNumbers, inputParticipantRecord, testFilesPath, 'AMENDED', false);
    await processFileViaStorage(parquet);
  ...
  test('@DTOSS-7663-01 ADD: blocked ineligible becomes eligible (no NBO)', async ({ request }, testInfo) => {
    ...
    // Seed ineligible first, then block
    ...
    // Now process the original input (which flips eligibility)
