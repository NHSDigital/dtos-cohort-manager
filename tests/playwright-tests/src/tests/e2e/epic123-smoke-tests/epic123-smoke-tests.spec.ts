...
testWithAmended('@DTOSS-5589-01 @smoke @e2e @ds @Implement Validate Amend fields reason for removal as null and date of death present', async ({ request, testData }) => {
  await test.step(`Given database does not contain record that will be processed`, async () => {
    await cleanupDatabaseFromAPI(request, testData.nhsNumbers);
  });
...
testWithAmended('@DTOSS-5407-01 @smoke @e2e @ds @Implement Validate Amend fields reason for removal as invalid and date of death present', async ({ request, testData }) => {
  await test.step(`Given database does not contain record that will be processed`, async () => {
    await cleanupDatabaseFromAPI(request, testData.nhsNumbers);
  });
