...
test('@DTOSS-5104-01 @regression @e2e @epic2-high-priority @Implement Validation for Eligibility Flag for Add', async ({ request, testData }) => {
...
test('@DTOSS-5613-01 @regression @e2e @epic2-high-priority @Implement Validation participant for Eligibility Flag set to true for Add', async ({ request, testData }) => {
  await test.step(`Then only one NHS Numbers should be updated in the cohort for elgibility flag true`, async () => {
    await validateSqlDatabaseFromAPI(request, testData.checkInDatabase);
  });
});

testWithAmended('@DTOSS-5605-01 @regression @e2e @epic2-high-priority @Implement Validation for Eligibility Flag for Amended', async ({ request, testData }) => {
...
testWithAmended('@DTOSS-4396-01 @regression @e2e @epic2-high-priority @Implement validate invalid flag value for Amended', async ({ request, testData }) => {
...
test('@DTOSS-4395-01 @regression @e2e @epic2-high-priority @Implement validate invalid flag set to false for ADD', async ({ request, testData }) => {
  await test.step(`Then validate invalid flag is set to false for ADD`, async () => {
    await validateSqlDatabaseFromAPI(request, testData.checkInDatabase);
  });
});

test('@DTOSS-4397-01 @regression @e2e @epic2-high-priority @Implement validate invalid flag set to true for ADD', async ({ request, testData }) => {
  await test.step(`Then validate invalid flag is set to true for ADD`, async () => {
    await validateSqlDatabaseFromAPI(request, testData.checkInDatabase);
  });
});

testWithAmended('@DTOSS-5419-01 @regression @e2e @epic2-high-priority @validate Gp practice codes and reason for removal no exception for Amended', async ({ request, testData }) => {
  await test.step(`Then ADD record should be updated in the cohort`, async () => {
    await validateSqlDatabaseFromAPI(request, testData.checkInDatabaseAdd);
  });
...
test('@DTOSS-4562-01 @regression @e2e @epic2-high-priority @Implement Validation for Eligibility Flag for Add set to true', async ({ request, testData }) => {
...
test('@DTOSS-3206-01 @regression @e2e @epic2-high-priority @Duplicate NHS ID Participants records', async ({ request, testData }) => {
