...
testWithDelete('@DTOSS-7614-01 AC01 Verify block a participant not processed to COHORT - Delete', async ({ request, testData }, testInfo) => {
    ...
    await test.step(`Then participant should be in the participant management table`, async () => {
      await validateSqlDatabaseFromAPI(request, testData.runTimeParquetFileAdd);
    });
