...
test.describe('@regression @e2e @epic4b-block-tests @smoke Tests', async () => {
  ...
  test('@DTOSS-7678-01 - AC1 - Verify participant is deleted from CohortDistributionDataService', async ({ request }: { request: APIRequestContext }, testInfo: TestInfo) => {
    ...
    await test.step(`verify that the participant is unblocked and then marked as eligible `, async () => {
      ...
      for(let i =0; i<10; i++) {
          ...
          if (responseData?.BlockedFlag === 0 && responseData?.EligibilityFlag === 1) {
            blocked = false;
            eligibility = true;
