...
public class ValidationExceptionDataTests
{
    ...
    public ValidationExceptionDataTests()
    {
        ...
        {
            new() { ExceptionId = 1, CohortName = "Cohort1", DateCreated = DateTime.Today.AddDays(-2), NhsNumber = "1111111111", RuleDescription = "RuleA", Category = 1, ServiceNowId = null },
            ...
            new() { ExceptionId = 3, CohortName = "Cohort3", DateCreated = DateTime.Today, NhsNumber = "3333333333", RuleDescription = "RuleC", Category = 3, ServiceNowId = "ServiceNow3" }
        };
    }

    #region GetAllExceptions
    [TestMethod]
    public async Task GetAllExceptions_TodaysExceptionsOnly_ReturnsTodaysExceptions()
    {
        // Arrange
        ...
        // Act
        var result = await validationExceptionData.GetAllExceptions(true, null);
    ...
    [TestMethod]
    public async Task GetAllExceptions_NoTodaysExceptionsOnly_ReturnsAllExceptions()
    {
        // Arrange
        var filteredList = _exceptionList;
        _validationExceptionDataServiceClient.Setup(x => x.GetAll()).ReturnsAsync(filteredList);

        // Act
        var result = await validationExceptionData.GetAllExceptions(false, null);
    ...
    public async Task GetAllExceptions_SortByDateCreatedOldest_ReturnsAllExceptionsInDateAscendingOrder()
    {
        ...
        // Act
        var result = await validationExceptionData.GetAllExceptions(false, ExceptionSort.DateCreatedOldest);
    ...
    public async Task GetAllExceptions_SortByExceptionStatusRaised_ReturnsAllExceptionsByRaisedStatusThenDateDescendingOrder()
    {
        ...
        // Act
        var result = await validationExceptionData.GetAllExceptions(false, ExceptionSort.ExceptionStatusRaised);

        // Assert
        result.Should().NotBeNull();
        result.Should().BeOfType<List<ValidationException>>();
        result.Should().HaveCount(3);
        result[0].ServiceNowId.Should().Be("ServiceNow3");
        result[1].ServiceNowId.Should().Be("ServiceNow2");
        result[2].ServiceNowId.Should().Be(null);
    }
    ...
    public async Task GetAllExceptions_SortByExceptionStatusNotRaised_ReturnsAllExceptionsByNotRaisedStatusThenDateDescendingOrder()
    {
        ...
        // Act
        var result = await validationExceptionData.GetAllExceptions(false, ExceptionSort.ExceptionStatusNotRaised);
        ...
        result[1].ServiceNowId.Should().Be("ServiceNow3");
