...
public class HttpClientFunctionTests
{
    ...
    [TestMethod]
    public async Task Run_SendGetResponseIsSuccessful_ReturnsOkResponse()
    {
        // Arrange
        _httpMessageHandler
            .Protected()
        ...
        // Act
        var result = await _function.SendGetResponse(_mockUrl, _mockParameters);
    ...
    [TestMethod]
    public async Task Run_SendGetResponseFails_LogsErrorWithoutNhsNumberAndThrowsException()
    {
        // Arrange
        var errorMessage = "There was an error";
        var nhsNumber = "1234567890";
        ...
        // Act & Assert
        var result = await Assert.ThrowsExceptionAsync<Exception>(() => _function.SendGetResponse(mockUrl, _mockParameters));
        Assert.AreEqual(errorMessage, result.Message);
